#!/usr/bin/env bash
# todo 프로젝트 소개 추가
# 1) 현재 스크립트가 위치한 디렉토리(보통 client/scripts)에서
#    상위 디렉토리(client)로 이동.
# 2) node_modules, dist, .idea, package-lock.json, fonts 등의 디렉토리/파일을 제외하고
#    특정 확장자(.js, .ts, .css, .html, .json, .sh, 등)를 하나의 파일(scripts/combined.txt)로 병합.
# 3) 최종적으로 "scripts/combined.txt"에 모든 소스가 합쳐지고,
...
##############################################
OUTPUT_FILE="scripts/combined.txt"
